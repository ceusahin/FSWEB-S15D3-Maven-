...
public class WordCounter {
    public static Map<String,Integer> calculatedWord()
    {
        ...
        Map<String, Integer> repatingWords = new HashMap<>();

        String[] words = text.toLowerCase(Locale.ENGLISH).replaceAll("[^a-zA-Z\\s]","").split("\\s");
        for (int i = 0; i < words.length; i++) {
            ...
            repatingWords.put(words[i],sameWordCounter);
        }
